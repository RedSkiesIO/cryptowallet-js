                ...
                */
                ...
                getUTXOs(addresses, network) {
                    return new Promise((resolve, reject) => {
                        ...
                        insight.getUnspentUtxos(addresses, function (error, utxos) {
                            if (error) {
                                //any other error
                                console.log(error);
                 ...
                 * @param amount
                 */
                createRawTx(accounts, change, utxos, entropy, network, toAddress, amount) {
                    const unit = Explorers.Unit;
                    const feeRate = 128;
                            ...
                            inputs.forEach((input) => {
                                accounts.forEach((account) => {
                                    if (input.address === account.address) {
                                        const key = this.generateKeyPair(wallet, account.index);
                                        if (account.change) {
                                            const key = this.generateKeyPair(wallet, account.index, true);
                                        }
