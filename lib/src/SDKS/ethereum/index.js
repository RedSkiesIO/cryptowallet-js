        ...
        (function (Ethereum) {
            class EthereumSDK extends GenericSDK_1.default {
                ...
                generateKeyPair(wallet, index) {
                    const addrNode = this.Bip.fromExtendedKey(wallet.external.xpriv).deriveChild(index);
                    const keypair = {
                ...
                */
                getTransactionFee(network) {
                    return new Promise((resolve, reject) => {
                        ...
                        this.axios.get(URL)
                            ...
                            .catch((e) => reject(new Error(`Failed to get transaction fee: ${e.message}`)));
                    });
                }
                /**
                ...
                createEthTx(keypair, toAddress, amount, gasPrice) {
                    ...
                    return new Promise((resolve, reject) => {
                        web3.eth.getTransactionCount(keypair.address, 'latest', (err, nonce) => {
                            if (err) {
                                return reject(err);
                            }
                            const sendAmount = amount.toString();
                            const gasAmount = gasPrice.toString();
                            const tx = new ethereumjs_tx_1.default({
                                nonce,
                ...
                verifyTxSignature(tx) {
                    const transaction = new ethereumjs_tx_1.default(tx);
                    this.VerifyTx = tx;
                 ...
                 * @param internal
                 */
                accountDiscovery(wallet, network, internal) {
                    const accounts = [];
