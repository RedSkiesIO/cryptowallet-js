...
(function (CryptoWallet) {
    ...
    (function (SDKS) {
        class GenericSDK {
            ...
            getWalletHistory(addresses, network, lastBlock, full) {
                ...
                return new Promise(async (resolve, reject) => {
                    addresses.forEach(async (address) => {
                        if (full) {
                            const history = this.getTransactionHistory(address, network, lastBlock, undefined, 50);
                            if (history.hasMore) {
                                let more = true;
                                let lBlock = history.lastBlock;
                                while (more) {
                                    const nextData = await this.getTransactionHistory(address, 'BITCOIN_TESTNET', 0, lBlock);
            ...
            getTransactionHistory(address, network, lastBlock, beforeBlock, limit) {
