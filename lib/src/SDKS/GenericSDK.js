...
(function (CryptoWallet) {
    ...
    (function (SDKS) {
        class GenericSDK {
            ...
            * @param amount
            */
            createRawTx(accounts, change, utxos, wallet, toAddress, amount, minerRate) {
                if (!wallet.network.connect) {
                    throw new Error('Invalid wallet type');
                }
            ...
            */
            ...
            getHistoricalData(coin, currency, period) {
                ...
                if (period === 'week') {
                    URL = `https://min-api.cryptocompare.com/data/histohour?fsym=${coin}&tsym=${currency}&limit=168&api_key=${this.networks.cryptocompare}`;
                }
                if (period === 'month') {
                    URL = `https://min-api.cryptocompare.com/data/histoday?fsym=${coin}&tsym=${currency}&limit=31&api_key=${this.networks.cryptocompare}`;
                }
                return new Promise((resolve, reject) => {
                    this.axios.get(URL)
                        .then((r) => {
