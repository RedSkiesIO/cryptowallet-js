...
const erc20_1 = require("../../src/SDKS/erc20");
const { assert } = Chai;
const { expect } = Chai;
const eth = SDKFactory_1.CryptoWallet.createSDK('Ethereum');
const erc20 = new erc20_1.default();
...
describe('ethereumSDK (wallet)', () => {
    ...
    it('can restore a wallet from the mnemonic', async () => {
        const accounts = await eth.accountDiscovery(entropy, 'ETHEREUM_ROPSTEN');
    ...
    it('can get the transaction history of a wallet', async () => {
        ...
        for (let i = 0; i < 10; i += 1) {
            const key = eth.generateKeyPair(wallet, i);
    ...
    it('can create a transaction', async () => {
        const wallet = eth.generateHDWallet(entropy, network);
        const keypair = eth.generateKeyPair(wallet, 0);
        const receiver = eth.generateAddress(wallet, 1);
