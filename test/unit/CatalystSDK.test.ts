...
const eth: any = CryptoWallet.createSDK('Catalyst');
const entropy = 'nut mixture license bean page mimic iron spice rail uncover then warfare';
const network = 'CATALYST_TESTNET';

describe('catalystSDK (wallet)', () => {
  describe('generateKeyPair', () => {
    it('can generate an catalyst testnet keypair', () => {
      const wallet = eth.generateHDWallet(entropy, network);
      const keypair = eth.generateKeyPair(wallet, 0);
      expect(keypair.address).toBe('0x0f7b9b42b1eb9246559b45367fa1739e41f0063b');
    });
  ...
  describe('generateAddress', () => {
    it('can generate an catalyst testnet address', () => {
      const wallet = eth.generateHDWallet(entropy, network);
      const account = eth.generateAddress(wallet, 0);
      expect(account.address).toBe('0x0f7b9b42b1eb9246559b45367fa1739e41f0063b');
    });
  ...
  describe('getTransactionFee', () => {
    it('can get the transacion fee for a catalyst transaction', async () => {
      const responseObj = {
        data: {
          name: 'ETH.main',
          height: 7270977,
  ...
  describe('createEthTx', () => {
    it('can create an catalyst testnet raw transaction', async () => {
      mockWeb3.eth.getTransactionCount.mockResolvedValue('0x5');
      const wallet = eth.generateHDWallet(entropy, network);
      const keypair = eth.generateKeyPair(wallet, 0);
  ...
  describe('broadcastTx', () => {
    it('can broadcast an catalyst testnet raw transaction', async () => {
  ...
  describe('getTransactionHistory', () => {
    // it('can get the transaction history of an catalyst address', async () => {
  ...
  describe('getBalance', () => {
    // it('can get the balance of an catalyst address', async () => {
  ...
  describe('accountDiscovery', () => {
    it('can discover an catalyst wallet', async () => {
      const wallet = eth.generateHDWallet(entropy, network);
